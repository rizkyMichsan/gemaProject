...
class Auth extends CI_Controller
{
	...
	public function login()
	{
		...
		{
			if(strpos($this->input->post('identity'), "@")>0){
				$d=explode("@", $this->input->post('identity'));
				$identi=$d[0];
			}
			else{
				$identi=$this->input->post('identity');
			}
			// check to see if the user is logging in
			// check for "remember me"
			$remember = (bool)$this->input->post('remember');

			if ($this->ion_auth->login($identi, $this->input->post('password'), $remember))
			{
	...
	public function ganti_password()
	{
		...
		if (!$this->ion_auth->logged_in())
		{
			...
			$user = $this->ion_auth->user($this->input->post('stafid'))->row();
	...
	public function create_user()
	{
		...
		{
			$this->form_validation->set_rules('email', $this->lang->line('create_user_validation_email_label'), 'trim|required|valid_email|is_unique[' . $tables['ic_users'] . '.email]');
		}
