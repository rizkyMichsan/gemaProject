...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$this->db->update($this->tables['ic_users'], $data, array('id' => $id));
		}
		else
		{
			$data = array(
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($this->db->count_all_results($this->tables['ic_users']) > 0)
		{
			$data = array(
			...
			$this->db->update($this->tables['ic_users'], $data, array('forgotten_password_code' => $code));
	...
	public function reset_password($identity, $new) {
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['ic_users'], $data, array($this->identity_column => $identity));
	...
	public function change_password($identity, $old, $new)
	{
		...
		if ($old_password_matches === TRUE)
		{
			...
			$successfully_changed_password_in_db = $this->db->update($this->tables['ic_users'], $data, array($this->identity_column => $identity));
	...
	public function username_check($username = '')
	{
		...
		return $this->db->where('username', $username)
						->limit(1)
						->count_all_results($this->tables['ic_users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->db->update($this->tables['ic_users'], $update, array($this->identity_column => $identity));
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$query = $this->db->get_where($this->tables['ic_groups'], array('name' => $this->config->item('default_group', 'ion_auth')), 1)->row();
		...
		$user_data = array_merge($this->_filter_data($this->tables['ic_users'], $additional_data), $data);

		$this->trigger_events('extra_set');

		$this->db->insert($this->tables['ic_users'], $user_data);

		$id = $this->db->insert_id($this->tables['ic_users'] . '_id_seq');
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			$qres = $this->db->get($this->tables['ic_login_attempts']);
	...
	public function get_last_attempt_time($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			$qres = $this->db->get($this->tables['ic_login_attempts'], 1);
	...
	public function increase_login_attempts($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			return $this->db->insert($this->tables['ic_login_attempts'], $data);
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			return $this->db->delete($this->tables['ic_login_attempts']);
		}
		return FALSE;
	}
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select(array(
			    $this->tables['ic_users'].'.*',
			    $this->tables['ic_users'].'.id as id',
			    $this->tables['ic_users'].'.id as user_id'
			));
		}
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				$this->db->distinct();
				$this->db->join(
				    $this->tables['ic_users_groups'],
				    $this->tables['ic_users_groups'].'.'.$this->join['ic_users'].'='.$this->tables['ic_users'].'.id',
				    'inner'
				);
			}
			...
			{
				$this->db->join($this->tables['ic_groups'], $this->tables['ic_users_groups'] . '.' . $this->join['ic_groups'] . ' = ' . $this->tables['ic_groups'] . '.id', 'inner');
				$this->db->where_in($this->tables['ic_groups'] . '.name', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->db->{$or_where_in}($this->tables['ic_users_groups'].'.'.$this->join['ic_groups'], $group_ids);
			}
		}
		...
		$this->response = $this->db->get($this->tables['ic_users']);
	...
	public function user($id = NULL)
	{
		...
		$this->limit(1);
		$this->order_by($this->tables['ic_users'].'.id', 'desc');
		$this->where($this->tables['ic_users'].'.id', $id);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['ic_users_groups'].'.'.$this->join['ic_groups'].' as id, '.$this->tables['ic_groups'].'.name, '.$this->tables['ic_groups'].'.description')
		                ->where($this->tables['ic_users_groups'].'.'.$this->join['ic_users'], $id)
		                ->join($this->tables['ic_groups'], $this->tables['ic_users_groups'].'.'.$this->join['ic_groups'].'='.$this->tables['ic_groups'].'.id')
		                ->get($this->tables['ic_users_groups']);
	}
	...
	public function add_to_group($group_ids, $user_id = FALSE)
	{
		...
		foreach ($group_ids as $group_id)
		{
			...
			if ($this->db->insert(
								  $this->tables['ic_users_groups'],
								  array(
								  	$this->join['ic_groups'] => (float)$group_id,
									$this->join['ic_users']  => (float)$user_id
								  )
								)
			)
			{
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		if (!empty($group_ids))
		{
			...
			foreach ($group_ids as $group_id)
			{
				...
				$this->db->delete(
					$this->tables['ic_users_groups'],
					array($this->join['ic_groups'] => (float)$group_id, $this->join['ic_users'] => (float)$user_id)
				);
		...
		{
			// Cast to float to support bigint data type
			if ($return = $this->db->delete($this->tables['ic_users_groups'], array($this->join['ic_users'] => (float)$user_id)))
			{
	...
	public function groups()
	{
		...
		$this->response = $this->db->get($this->tables['ic_groups']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$this->where($this->tables['ic_groups'].'.id', $id);
		}
	...
	public function update($id, array $data)
	{
		...
		$data = $this->_filter_data($this->tables['ic_users'], $data);
		...
		$this->trigger_events('extra_where');
		$this->db->update($this->tables['ic_users'], $data, array('id' => $user->id));
	...
	public function delete_user($id)
	{
		...
		$this->db->delete($this->tables['ic_users'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['ic_users'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$this->db->update($this->tables['ic_users'], array('remember_code' => $salt), array('id' => $id));
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$existing_group = $this->db->get_where($this->tables['ic_groups'], array('name' => $group_name))->num_rows();
		if($existing_group !== 0)
		{
		...
		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['ic_groups'], $additional_data), $data);

		$this->trigger_events('extra_group_set');

		// insert the new group
		$this->db->insert($this->tables['ic_groups'], $data);
		$group_id = $this->db->insert_id($this->tables['ic_groups'] . '_id_seq');
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = array())
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->db->get_where($this->tables['ic_groups'], array('name' => $group_name))->row();
		...
		$group = $this->db->get_where($this->tables['ic_groups'], array('id' => $group_id))->row();
		...
		{
			$data = array_merge($this->_filter_data($this->tables['ic_groups'], $additional_data), $data);
		}

		$this->db->update($this->tables['ic_groups'], $data, array('id' => $group_id));
	...
	public function delete_group($group_id = FALSE)
	{
		...
		$this->db->delete($this->tables['ic_users_groups'], array($this->join['ic_groups'] => $group_id));
		// remove the group itself
		$this->db->delete($this->tables['ic_groups'], array('id' => $group_id));
